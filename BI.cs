    ...
    class BI
    {
        ...
        public string getObjUrl(baseClass obj, string gatewayURI)
        {
            switch (obj.objectClass.value.ToString()) {
                ...
                case "jobDefinition":
                    jobDefinition j=loadJobData((jobDefinition) obj);
                    return gatewayURI + 
                        "?b_action=xts.run&" +
                        "m=portal%2FrunWithOptions%2FjobDefinition.xts&" +
                        "m_class=jobDefinition&" +
                        "m_name=" + HttpUtility.UrlEncode(j.name.value[0].value) + "&" +
                        "m_obj=" + Uri.EscapeDataString(j.searchPath.value) + "&" +
                        "m_path=" + Uri.EscapeDataString(j.parent.value[0].searchPath.value);
                default:
                    return "#";
            }
        }
        ...
        protected jobDefinition loadJobData(jobDefinition obj)
        {
            propEnum[] props = {
                                   propEnum.name,
                                   propEnum.searchPath,
            ...
            return (jobDefinition)loadObjData(obj, props);
        }
